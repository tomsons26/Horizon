...
void Debug_Init(int flags)
{
    ...
    if ( DebugFlags & DEBUG_LOG_TO_FILE ) {
        ...
        if ( tmp != NULL ) {
            strcpy(dirbuf, tmp);
            strcat(dirbuf, "\\Documents\\Horizon");
            mkdir(dirbuf);
            strcat(dirbuf, "\\");
        } else {
        ...
        strcat(prevbuf, "HorizonDebugLogPrev.txt");
        strcpy(curbuf, dirbuf);
        strcat(curbuf, prefix);
        strcat(curbuf, "HorizonDebugLogFile.txt");
        remove(prevbuf);
